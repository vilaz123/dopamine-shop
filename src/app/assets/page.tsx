...
export default function AssetsPage() {
  ...
  return (
    ...
    <section className="container-shell py-16">
      ...
      <section className="mt-14"><h2 className="font-display mb-6 text-4xl">虚拟囤货库存</h2><div className="grid gap-5 md:grid-cols-3 lg:grid-cols-4">{inventoryProducts.length ? inventoryProducts.map(({ product, count }) => <div key={product.slug} className="rounded-[2rem] bg-white p-4"><ProductMonogram product={product} /><p className="font-display mt-4 text-2xl">{product.name}</p><p className="text-sm text-[var(--muted)]">已拥有 ×{count}</p></div>) : <p className="text-[var(--muted)]">还没有囤货。</p>}</div></section>
