...
export default function AssetsPage() {
  ...
  return (
    ...
    <section className="container-shell py-10 sm:py-16">
      ...
      <h1 className="font-display mt-3 text-3xl sm:text-4xl" style={{ color: "var(--page-ink)" }}>我的多巴胺资产</h1>
      <div className="mt-8 grid grid-cols-1 gap-4 sm:gap-6 sm:mt-10 md:grid-cols-3">
        <div className="dopamine-panel rounded-[2rem] p-7"><p className="text-sm text-white/70">虚拟钱包</p><p className="font-display mt-2 text-5xl">{coins} 币</p></div>
        <div className="rounded-[2rem] border border-white/50 bg-white/65 p-5 backdrop-blur sm:p-7"><p className="text-sm text-[var(--muted)]">会员等级</p><p className="font-display mt-2 text-4xl sm:text-5xl">Lv.{level.level}</p><p className="mt-2 text-sm text-[var(--muted)]">{level.title} · {xp}/{level.xpToNext} XP</p></div>
        <div className="rounded-[2rem] border border-white/50 bg-white/65 p-5 backdrop-blur sm:p-7"><p className="text-sm text-[var(--muted)]">优惠券</p><p className="font-display mt-2 text-4xl sm:text-5xl">{coupons.length} 张</p><p className="mt-2 text-sm text-[var(--muted)]">可在虚拟结算中使用</p></div>
      </div>

      <section className="mt-14"><h2 className="font-display mb-6 text-4xl">我的勋章</h2><div className="flex flex-wrap gap-3">{badges.length ? badges.map((badge) => <span key={badge.id} className="rounded-full border border-white/50 bg-white/65 px-5 py-3 text-sm backdrop-blur">{badge.icon} {badge.name}</span>) : <p className="text-[var(--muted)]">还没有勋章，下一单就会解锁。</p>}</div></section>

      <section className="mt-12 sm:mt-14"><h2 className="font-display mb-5 text-3xl sm:mb-6 sm:text-4xl">虚拟囤货库存</h2><div className="grid grid-cols-2 gap-4 sm:gap-5 md:grid-cols-3 lg:grid-cols-4">{inventoryProducts.length ? inventoryProducts.map(({ product, count }) => <div key={product.slug} className="rounded-[1.25rem] border border-white/50 bg-white/65 p-3 backdrop-blur sm:rounded-[2rem] sm:p-4"><ProductMonogram product={product} /><p className="font-display mt-3 text-xl sm:mt-4 sm:text-2xl">{product.name}</p><p className="text-sm text-[var(--muted)]">已拥有 ×{count}</p></div>) : <p className="text-[var(--muted)]">还没有囤货。</p>}</div></section>
