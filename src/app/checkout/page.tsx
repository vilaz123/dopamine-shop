...
export default function CheckoutPage() {
  ...
  const subtotal = lines.reduce((sum, line) => sum + line.product.price * line.item.quantity + (line.item.giftWrap || giftWrap ? 9 : 0), 0);
  ...
  function submit() {
    ...
    const order = {
      ...
      items: lines.map(({ item, product }) => ({ ...item, giftWrap: item.giftWrap || giftWrap, name: product.name, subtitle: product.subtitle, price: product.price, accent: product.accent, saturation: product.saturation, monogram: product.monogram, rewardCoins: product.rewardCoins, deliveryFlavor: product.deliveryFlavor ?? "parcel" as const })),
      subtotal,
      discount,
      total,
      coinsEarned,
