    ...
    <section className="container-shell py-10 sm:py-16">
      ...
      <div className="mt-8 grid gap-6 sm:gap-10 lg:grid-cols-[1fr_420px]">
        <div className="space-y-5 sm:space-y-6">
          <div className="rounded-[1.25rem] bg-white p-5 sm:rounded-[2rem] sm:p-7"><h2 className="font-display text-3xl sm:text-4xl">虚拟地址</h2><input value={virtualAddress} onChange={(e) => setVirtualAddress(e.target.value)} className="mt-4 w-full rounded-2xl border border-black/10 bg-white/65 px-4 py-3 text-sm outline-none sm:mt-5" /></div>
          ...
          <div className="rounded-[1.25rem] bg-white p-5 sm:rounded-[2rem] sm:p-7"><h2 className="font-display text-3xl sm:text-4xl">备注留言</h2><Textarea className="mt-4 sm:mt-5" rows={4} value={note} onChange={(e) => setNote(e.target.value)} placeholder="例如：请把快乐放在门口，不要真实敲门。" /></div>
        </div>
        <aside className="h-fit rounded-[1.5rem] bg-[var(--ink)] p-5 text-[var(--bone)] sm:rounded-[2rem] sm:p-7"><h2 className="font-display text-3xl sm:text-4xl">订单确认</h2><div className="mt-5 space-y-3 sm:mt-6 sm:space-y-4">{lines.map(({ item, product }) => <div key={`${item.slug}-${JSON.stringify(item.options)}`} className="flex justify-between gap-3 border-b border-white/10 pb-3 text-sm text-white/70 sm:gap-4 sm:pb-4"><span>{product.name} × {item.quantity}</span><span>{formatCurrency((product.price + (item.optionPriceDelta ?? 0)) * item.quantity)}</span></div>)}</div><div className="mt-5 space-y-3 text-sm text-white/70 sm:mt-6"><div className="flex justify-between"><span>虚拟小计</span><span>{formatCurrency(subtotal)}</span></div><div className="flex justify-between"><span>优惠</span><span>-{formatCurrency(discount)}</span></div><div className="flex justify-between border-t border-white/10 pt-3 text-[var(--bone)] sm:pt-4"><span>实际扣款</span><span className="font-display text-3xl sm:text-4xl">¥0</span></div><p className="text-[var(--gold)]">下单后获得 +{coinsEarned} 币 · +{xpEarned} XP</p></div><Button type="button" variant="light" className="mt-6 w-full sm:mt-8" onClick={submit}>一键虚拟下单</Button></aside>
      </div>
    </section>
    </PageTheme>
  );
