    ...
    return (
      <section className="container-shell py-16">
        <div className="rounded-[2.5rem] border border-dashed border-black/15 bg-[#FFFFFF] p-12 text-center">
          <p className="text-xs uppercase tracking-[0.32em] text-[#FF3D81]">Orders</p>
          <h1 className="font-display mt-4 text-6xl">还没有虚拟订单。</h1>
          <p className="mt-5 text-[#5A4A6A]">当购买欲出现时，可以先在 Dopahub 多巴胺仓下一单。</p>
    ...
    <section className="container-shell py-16">
      ...
      <div className="mt-12 grid gap-10 lg:grid-cols-[360px_1fr]">
        <aside className="space-y-4">
          {orders.map((order) => (
            <button key={order.id} onClick={() => setSelectedId(order.id)} className={`w-full rounded-[1.5rem] border p-5 text-left transition ${selected?.id === order.id ? "border-black bg-[#241A4D] text-[#FFF5F8]" : "border-black/10 bg-[#FFFFFF]"}`}>
...
function OrderDetail({ order, onSign }: { order: ReturnType<typeof useOrderStore.getState>["orders"][number]; onSign: () => void }) {
  ...
  return (
    <div className="rounded-[2.5rem] bg-[#FFFFFF] p-8">
      <div className="flex flex-col justify-between gap-6 border-b border-black/10 pb-8 md:flex-row md:items-start">
        <div><p className="text-sm text-[#5A4A6A]">订单编号</p><h2 className="font-display mt-2 text-5xl">{order.id}</h2><p className="mt-4 text-[#5A4A6A]">当前状态：<span className="text-black">{current.label}</span></p></div>
        <div className="rounded-3xl bg-[#241A4D] p-6 text-[#FFF5F8]"><p className="text-sm text-white/60">本单奖励</p><p className="font-display mt-2 text-4xl">+{order.coinsEarned} 币</p><p className="text-sm text-white/60">+{order.xpEarned} XP</p></div>
      </div>
      {order.deliveryFlavor === "rider" && (
        <div className="mt-8 rounded-3xl bg-[#241A4D] p-6 text-[#FFF5F8]">
          <p className="font-display text-4xl">{current.label}</p>
          <p className="mt-2 text-sm text-white/70">骑手正在幻想路线中接近你，永远差一栋楼</p>
        </div>
      )}
      <div className="mt-8 grid gap-10 lg:grid-cols-[1fr_1fr]">
        <div><h3 className="font-display text-4xl">虚拟战利品</h3><div className="mt-5 space-y-4">{order.items.map((item) => <div key={`${item.slug}-${JSON.stringify(item.options)}`} className="rounded-2xl border border-black/10 p-4"><div className="flex justify-between gap-4"><div><p className="font-display text-2xl">{item.name}</p><p className="text-sm text-[#5A4A6A]">{Object.values(item.options).join(" / ")} · ×{item.quantity}{item.giftWrap ? " · 礼品包装" : ""}</p></div><p>{formatCurrency(item.price * item.quantity)}</p></div></div>)}</div><div className="mt-8 rounded-2xl border border-black/10 p-5 text-sm leading-7 text-[#3D3357]"><p>虚拟地址：{order.profile.virtualAddress}</p><p>优惠券：{order.profile.couponLabel ?? "未使用"}</p>{order.profile.note && <p>备注：{order.profile.note}</p>}{order.badges.length > 0 && <p>勋章：{order.badges.join("、")}</p>}</div></div>
