...
export default function OrdersPage() {
    ...
    return (
      ...
      <section className="container-shell py-16">
        <div className="rounded-[2.5rem] border border-dashed border-black/15 bg-white p-12 text-center">
          ...
          <h1 className="font-display mt-4 text-6xl" style={{ color: "var(--page-ink)" }}>还没有虚拟订单。</h1>
  ...
  return (
    ...
    <section className="container-shell py-16">
      ...
      <p className="text-xs uppercase tracking-[0.32em]" style={{ color: "var(--page-ink)" }}>Orders</p>
      <h1 className="font-display mt-4 text-6xl" style={{ color: "var(--page-ink)" }}>我的虚拟订单</h1>
...
function OrderDetail({ order, onSign }: { order: ReturnType<typeof useOrderStore.getState>["orders"][number]; onSign: () => void }) {
  ...
  return (
    <div className="rounded-[2.5rem] border border-white/60 bg-white/85 p-8 shadow-sm">
      <div className="flex flex-col justify-between gap-6 border-b border-black/10 pb-8 md:flex-row md:items-start">
        <div><p className="text-sm" style={{ color: "var(--page-soft)" }}>订单编号</p><h2 className="font-display mt-2 text-5xl" style={{ color: "var(--page-ink)" }}>{order.id}</h2><p className="mt-4" style={{ color: "var(--page-soft)" }}>当前状态：<span style={{ color: "var(--page-ink)" }}>{current.label}</span></p></div>
        <div className="rounded-3xl p-6 text-white" style={{ background: "var(--page-ink)" }}><p className="text-sm text-white/70">本单奖励</p><p className="font-display mt-2 text-4xl">+{order.coinsEarned} 币</p><p className="text-sm text-white/70">+{order.xpEarned} XP</p></div>
      </div>
      {order.deliveryFlavor === "rider" && (
        <div className="mt-8 rounded-3xl p-6 text-white" style={{ background: "var(--page-ink)" }}>
      ...
      <div className="mt-8 grid gap-10 lg:grid-cols-[1fr_1fr]">
        <div><h3 className="font-display text-4xl" style={{ color: "var(--page-ink)" }}>虚拟战利品</h3><div className="mt-5 space-y-4">{order.items.map((item) => <div key={`${item.slug}-${JSON.stringify(item.options)}`} className="rounded-2xl border border-black/10 p-4"><div className="flex justify-between gap-4"><div><p className="font-display text-2xl" style={{ color: "var(--page-ink)" }}>{item.name}</p><p className="text-sm" style={{ color: "var(--page-soft)" }}>{Object.values(item.options).join(" / ")} · ×{item.quantity}{item.giftWrap ? " · 礼品包装" : ""}</p></div><p style={{ color: "var(--page-ink)" }}>{formatCurrency(item.price * item.quantity)}</p></div></div>)}</div><div className="mt-8 rounded-2xl border border-black/10 p-5 text-sm leading-7" style={{ color: "var(--page-soft)" }}><p>虚拟地址：{order.profile.virtualAddress}</p><p>优惠券：{order.profile.couponLabel ?? "未使用"}</p>{order.profile.note && <p>备注：{order.profile.note}</p>}{order.badges.length > 0 && <p>勋章：{order.badges.join("、")}</p>}</div></div>
