...
export function EmailAuthForm() {
  ...
  return (
    <form onSubmit={submit} className="rounded-[2.5rem] bg-[#fffaf2] p-8">
      ...
      <p className="mt-3 text-sm leading-6 text-[#7a7167]">
        用邮箱 + 密码登录，订单、资产与社区记录会同步到云端并在多设备间共享。
        {cloudOff && <span className="text-[#8b6b2f]"> （未配置云端，当前为本地账号模式。）</span>}
      </p>
      ...
      {notice && <p className="mt-4 text-sm text-[#8b6b2f]">{notice}</p>}
      <Button type="submit" className="mt-6 w-full" disabled={busy}>
        {busy ? "处理中…" : mode === "signin" ? "登录 / 进入账号" : "注册并进入"}
      </Button>
      <button
        type="button"
        className="mt-4 w-full text-sm text-[#7a7167] underline-offset-4 hover:underline"
        onClick={() => {
          setError("");
          setNotice("");
