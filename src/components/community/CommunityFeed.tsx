...
export function CommunityFeed() {
  ...
  return (
    <div className="grid gap-10 lg:grid-cols-[360px_1fr]">
      <aside>
        <form onSubmit={submit} className="rounded-[2rem] bg-[#fffaf2] p-6">
        ...
        <div className="mt-6 flex flex-wrap gap-2">
          {communityTopics.map((item) => <button key={item} onClick={() => setTopic(item)} className={`rounded-full border px-3 py-2 text-sm ${topic === item ? "border-black bg-black text-[#f6f1e8]" : "border-black/10"}`}>#{item}</button>)}
        </div>
      </aside>
      <div className="space-y-5">
        {filtered.map((post) => (
          <article key={post.id} className="rounded-[2rem] border border-black/10 bg-[#fffaf2] p-6">
            <p className="text-xs uppercase tracking-[0.25em] text-[#8b6b2f]">#{post.topic}</p>
