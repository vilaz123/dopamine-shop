  ...
  return (
    <div className="rounded-[1.5rem] border border-black/10 bg-white/40 p-4">
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        {presets.map((code) => (
          <button key={code} onClick={() => onChange(code)} className={`rounded-full border px-3 py-1 text-xs transition ${value === code ? "border-black bg-black text-[#f6f1e8]" : "border-black/10 hover:border-black/40"}`}>{code}</button>
        ))}
      </div>
