...
export function GiftWrapToggle({ checked, onChange }: { checked: boolean; onChange: (checked: boolean) => void }) {
  return (
    <button onClick={() => onChange(!checked)} className={`w-full rounded-[1.5rem] border p-4 text-left transition ${checked ? "border-black bg-black text-[var(--bone)]" : "border-black/10 bg-white/40"}`}>
