    ...
    <div className="mt-4 space-y-3">
      {product.options.map((option) => (
        <div key={option.label}>
          <p className="mb-2 text-xs font-semibold text-[#7a7167]">{option.label}</p>
            ...
            {option.values.map((raw) => {
              ...
              return (
                <button
                  ...
                  className={`rounded-full border px-3 py-1 text-xs transition ${active ? "border-black bg-black text-[#f6f1e8]" : "border-black/10 hover:border-black/30"}`}
                >
                  {value}{delta ? ` +¥${delta}` : ""}
                </button>
              );
