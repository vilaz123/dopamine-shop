      ...
      {promos.map((promo) => {
        ...
        return (
          <button
            ...
            onClick={() => claim(promo)}
            disabled={done}
            className={`flex min-w-[220px] flex-col gap-1 rounded-2xl border p-4 text-left transition ${done ? "cursor-default border-black/10 bg-[#fffaf2] opacity-60" : "border-yellow-400/40 bg-yellow-400/15 hover:border-yellow-400/80 hover:bg-yellow-400/25"}`}
          >
            ...
            <span className="text-xs text-[#7a7167]">{done ? "已放进你的虚拟券包" : promo.hint}</span>
          </button>
        );
      })}
    </div>
