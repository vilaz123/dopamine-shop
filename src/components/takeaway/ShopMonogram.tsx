...
import { takeawayBucket } from "@/lib/data/takeaway-shops";
...
export function ShopMonogram({ shop, large = false, priority = false }: { shop: TakeawayShop; large?: boolean; priority?: boolean }) {
  const bucket = takeawayBucket(shop.category);
  if (shop.image) {
    return (
      <div className="relative h-full w-full overflow-hidden rounded-[1rem] sm:rounded-[2rem] luxury-shadow">
        ...
        <div className="absolute bottom-3 left-3 rounded-full bg-black/40 px-2.5 py-1 text-[10px] uppercase tracking-[0.2em] text-white/90 backdrop-blur-sm sm:bottom-4 sm:left-4 sm:text-xs">{bucket}</div>
      </div>
    );
  }
