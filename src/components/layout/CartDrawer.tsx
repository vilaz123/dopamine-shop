...
export function CartDrawer() {
  ...
  return (
    <div className="fixed inset-0 z-50">
      ...
      <aside className="absolute right-0 top-0 flex h-full w-full max-w-md flex-col bg-[#FFFFFF] p-6 shadow-2xl">
        <div className="flex items-center justify-between border-b border-black/10 pb-5">
          <h2 className="font-display text-3xl">多巴胺购物车</h2>
          <button onClick={() => setOpen(false)} className="rounded-full border border-black/10 p-2"><X size={18} /></button>
        </div>
        <div className="flex-1 overflow-auto py-6">
          {lines.length === 0 ? (
            <div className="rounded-3xl border border-dashed border-black/15 p-8 text-center text-[#5A4A6A]">购物车是空的，钱包也是安全的。</div>
          ) : (
                ...
                <div key={`${item.slug}-${JSON.stringify(item.options)}-${item.giftWrap}`} className="border-b border-black/10 pb-5">
                  <div className="flex justify-between gap-4">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-[#5A4A6A]">数量 {item.quantity} · {Object.values(item.options).join(" / ")}{item.giftWrap ? " · 礼品包装" : ""}</p>
                    </div>
                    <p className="font-display text-xl">{formatCurrency(product.price * item.quantity + (item.giftWrap ? 9 : 0))}</p>
                  </div>
                  <button onClick={() => removeItem(item.slug, item.options, item.giftWrap)} className="mt-3 text-xs uppercase tracking-[0.25em] text-[#5A4A6A] hover:text-black">移除</button>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="border-t border-black/10 pt-5">
          <div className="mb-3 flex items-center justify-between">
            <span className="text-sm text-[#5A4A6A]">虚拟金额</span>
            <span className="font-display text-3xl">{formatCurrency(total)}</span>
          </div>
          <p className="mb-5 text-sm text-[#FF3D81]">结算预计获得 +{rewardCoins} 多巴胺币，实际扣款 ¥0。</p>
