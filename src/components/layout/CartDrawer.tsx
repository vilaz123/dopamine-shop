      ...
      <aside className="absolute right-0 top-0 flex h-full w-full max-w-md flex-col bg-[#fffaf2] p-6 shadow-2xl">
            ...
            <div className="space-y-5">
              {lines.map(({ item, product }) => (
                <div key={`${item.slug}-${JSON.stringify(item.options)}-${item.giftWrap}`} className="border-b border-black/10 pb-5">
                  ...
                  <button onClick={() => removeItem(item.slug, item.options, item.giftWrap)} className="mt-3 text-xs uppercase tracking-[0.25em] text-[#7a7167] hover:text-black">移除</button>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="border-t border-black/10 pt-5">
          <div className="mb-3 flex items-center justify-between">
            <span className="text-sm text-[#7a7167]">虚拟金额</span>
            <span className="font-display text-3xl">{formatCurrency(total)}</span>
          </div>
          <p className="mb-5 text-sm text-[#8b6b2f]">结算预计获得 +{rewardCoins} 多巴胺币，实际扣款 ¥0。</p>
