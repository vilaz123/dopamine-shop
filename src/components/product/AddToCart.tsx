            ...
            {option.values.map((raw) => {
              ...
              return (
                <button
                  ...
                  className={`rounded-full border px-4 py-2 text-sm transition ${selected[option.label] === value ? "border-black bg-black text-[var(--bone)]" : "border-black/10 hover:border-black/30"}`}
                >
                  {value}{delta ? ` +¥${delta}` : ""}
                </button>
              );
